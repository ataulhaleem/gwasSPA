...
private:
	...
	void sort_eigen_vectors(sort_order order = sort_order::ascending)
	{
		...
		vector<pair<int, int>> eigen_value_index_vector;
		for (int i = 0; i < eigen_values.size(); ++i)
		{
			eigen_value_index_vector.push_back(make_pair(eigen_values[i], i));
		}

		if (order == sort_order::ascending)
			sort(begin(eigen_value_index_vector), end(eigen_value_index_vector), greater<pair<int, int>>());
		else
			sort(begin(eigen_value_index_vector), end(eigen_value_index_vector), less<pair<int, int>>());
